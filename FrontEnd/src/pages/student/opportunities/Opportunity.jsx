        ...
        <EuiFlexGroup justifyContent="flexStart" alignItems="flexStart">
          ...
          <EuiFlexItem grow={false}>
            <span
              ...
              className="opportunity-main-panel-tag"
              onClick={() => {
                history.replace(`/student/applicants/${data._id}`);
              }}
            >
        ...
        <EuiPanel style={{ marginTop: "1rem" }}>
          ...
          {data.selectionProcess.map((process,index)=>{
            return(
                  <EuiText key={index}>
                    <h4>
                      {process.name} {process.selected.length > 0 && (<a href={`http://localhost:3000/student/opportunity/${data._id}/selectionProcess/${process._id}/selected`}>- {process.selected.length} selected </a>)}
                    </h4>
                  </EuiText>
